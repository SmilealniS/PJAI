...
"""A module for local search algorithm to find a good Cart-Pole agent."""
import sys
...
def simulated_annealing(env, agent, init_temp=25.0, temp_step=-0.1, max_iters=500):
    """
    Run a hill-climbing search, and return the final agent.
    ...
    for __ in range(max_iters):
        ...
        init_temp = init_temp + temp_step   #   simply decrease temperature
        if init_temp == 0:
            return cur_agent, history

        # next = neighbors[np.random.randint(0, len(neighbors))]
        random_agent = np.random.choice(neighbors)
        rewards = simulate(env, [random_agent])[0]

        E = cur_r - rewards
        if E < 0:
            cur_r = rewards
            cur_agent = random_agent
            history.append(cur_r)
        elif(np.random.uniform(0, 1) <= np.exp(-E / init_temp)):
            cur_r = rewards
            cur_agent = random_agent
            history.append(cur_r)
